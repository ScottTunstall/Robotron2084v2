...
; Convention: 
;
; 1. In the documentation, zero based indexes are assumed unless specified otherwise.
;
; 2. Where the term METADATA is used, that means "information about data". For example, the term "Animation Frame Metadata" means "information held about an animation frame", which is
...
;
; The following EQUates define addresses of CMOS-backed Game settings that can be edited in the GAME ADJUSTMENT screen.
;
; For all settings *except the attract mode message*, the setting value is packed into 2 bytes (a word) where the lower nibble of each byte represents a single BCD digit (ie: 0-9). 
;
; Examples:
;  reading F0, F1 from CMOS would give you BCD number "01"
;  reading F1, F2 would give you BCD "12"
;
;
; The attract mode message occupies 50 bytes in CMOS. Two bytes are used per character in the message, giving 25 characters total.
; 
; Rg;  
;
; Examples: 
;  reading F4, F1 from the attract mode area of CMOS, would give you #$41, which is the ordinal for the character "A" 
; 
;
...
pricing_selection EQU $CC04                              ; 0 - 9. 9 = free play
     ...
     centre_slot_units EQU $CC08                         ; number of coins inserted into centre slot           
     right_slot_units EQU $CC0A                          ; number of coins inserted into right slot
...
auto_cycle EQU $CC1E
set_attract_mode_message EQU $CC20                       ; Boolean. 1 = set message, 0 = don't
set_highest_score_name EQU $CC22                         ; Boolean. 1 = set highest score name, 0 = don't
attract_mode_message EQU $CC24                           ; Text for attract mode message. Up to 50 characters (decimal) long, 2 bytes per char, so 100 bytes (decimal) in size.
    attract_mode_message_first_line EQU $CC24            ; First line of attract mode message
    attract_mode_message_second_line EQU $CC56           ; Second line of attract mode message  
    attract_mode_message_end EQU $CC87
...
;
; Robotron's character set is as follows:
; 0123456789 !,.:-ABCDEFGHIJKLMNOPQRSTUVWXYZ()/◄
;
; The first character in the set, "0", has an ordinal value of #$30 (48 decimal) - just like ASCII.
; Robotron's character set matches ASCII as far as the ordinals for A-Z and 0-9 are concerned. Everything else - no.
; e.g. the SPACE character is #$3A (58 decimal) in Robotron whereas in ASCII it is #$20 (32 decimal)
;
; The last character in the set ◄ is the graphic representation for BACKSPACE. It has an ordinal of #$5E .
;
; Robotron supports large and small versions of the character set.
;
...
;
; U = pointer to function to call when family member killed or rescued
...
02DC: 02 5F        ; pointer to ANIMATE_FAMILY_MEMBER function 
...
02F0: EC 63       LDD   $0003,S              ; D = pointer to routine to rescue/kill family member
02F2: ED 08       STD   $0008,X              ; store address 
...
116F: BD D0 6C    JSR   $D06C                ; JMP $D32B - create entity with params and add to linked list at $9817
; parameters to pass to $D06C 
1172: 11 AF       ; address of function to call after 1 game cycle
1174: 
...
710F: BD 74 78    JSR   $7478                ; call routine to calculate and save checksum for game settings in CMOS
...
719B: BD 74 78    JSR   $7478                ; call routine to calculate and save checksum for game settings in CMOS
...
71E5: 27 13       BEQ   $71FA                ; if A == 0 then no, its not a units setting, we can edit it. Clear carry flag and exit @$71FA.
71E7: 8E CC 04    LDX   #$CC04               ; X = address in CMOS of PRICING SELECTION 
71EA: 4A          DECA                       ; if this is indented, meaning its a "units setting".. DECA will make A 0 and...
...
71F4: 27 04       BEQ   $71FA                ; Yes, A is zero so clear carry flag and exit @$71FA. The user can edit the units.
...
7206: 86 2C       LDA   #$2C                 ; index of text string for "->"
...
WAIT_BEFORE_ALLOWING_MOVE_TO_NEXT_GAME_SETTING:
...
WAIT_BEFORE_ALLOWING_MOVE_TO_PREVIOUS_GAME_SETTING:
...
MOVE_CURSOR_TO_NEXT_GAME_SETTING:
...
7278: BD 71 E1    JSR   $71E1                ; test if can edit PRICING SELECTION units
727B: 25 F0       BCS   $726D                ; no, we can't edit units, goto $726D
...
MOVE_CURSOR_TO_PREVIOUS_GAME_SETTING:
...
7294: BD 71 E1    JSR   $71E1                ; test if can edit PRICING SELECTION units
7297: 25 F0       BCS   $7289                ; no, we can't edit units, goto $7289
...
72B9: 26 F9       BNE   $72B4                ; yes, goto $72B4 - repeat until the user takes his finger off the button
...
72C5: BD 74 78    JSR   $7478                ; call routine to calculate and save checksum for game settings in CMOS
...
72E0: BD 74 78    JSR   $7478                ; call routine to calculate and save checksum for game settings in CMOS
...
72F8: BD 74 78    JSR   $7478                ; call routine to calculate and save checksum for game settings in CMOS
; clear out existing attract mode message
72FB: 86 3A       LDA   #$3A                 ; Load a with ordinal for SPACE (in the Robotron character set, not ASCII)
72FD: 8E CC 24    LDX   #$CC24               ; start of attract mode message in CMOS
7300: C6 32       LDB   #$32                 ; attract mode message is 50 characters long
7302: BD D0 AB    JSR   STA_NIB_X1           ; Pack A into 2 bytes and write to CMOS
7305: 5A          DECB                       ; do until B==0
7306: 26 FA       BNE   $7302
...
7310: 10 8E CC 24 LDY   #$CC24               ; address of the attract mode message's first line of text in CMOS
7314: 8E 25 30    LDX   #$2530               ; screen address
7317: CC 19 80    LDD   #$1980
731A: BD 75 3A    JSR   $753A
731D: C6 30       LDB   #$30
731F: 8E CC 88    LDX   #$CC88
7322: 10 8E CC 24 LDY   #$CC24               ; address of the attract mode message's first line of text in CMOS
7326: BD 73 8A    JSR   $738A                ; call CENTER THE LINE function
...
732E: BD 74 0D    JSR   $740D                ; display message in new position

7331: 10 8E CC 56 LDY   #$CC56               ; address of the attract mode message's second line of text in CMOS
7335: 8E 25 40    LDX   #$2540
7338: CC 19 80    LDD   #$1980
733B: BD 75 3A    JSR   $753A
733E: C6 40       LDB   #$40
7340: 8E CC 8A    LDX   #$CC8A
7343: 10 8E CC 56 LDY   #$CC56               ; address of the attract mode message's second line of text in CMOS
7347: 8D 41       BSR   $738A                ; call CENTER THE LINE function
7349: BD 74 91    JSR   $7491
734C: 8E CC 8E    LDX   #$CC8E
734F: BD D0 AB    JSR   STA_NIB_X1           ; split A into 2 bytes, then write to X 
...
736C: BD 74 78    JSR   $7478                ; call routine to calculate and save checksum for game settings in CMOS
736F: 8D 08       BSR   $7379
...
; TODO: Identify what the purpose of each register is
;
;
; Expects:
; B:
; X: Pointer to 
; Y: Address of packed text characters

CENTRE_THE_LINE:
738A: DE 15       LDU   $15                  ; get address of current task into U                 
738C: 86 25       LDA   #$25                 ; initial value for horizontal position counter                 
738E: ED 47       STD   $0007,U
7390: 35 06       PULS  A,B                  ; pop return address off stack
7392: ED 4D       STD   $000D,U              ; store in task
7394: AF 4B       STX   $000B,U
7396: 10 AF 49    STY   $0009,U              ; address of packed characters 
7399: 86 25       LDA   #$25                 ; initial value for horizontal position counter
739B: BD D0 AB    JSR   STA_NIB_X1           ; split A into 2 and write result to X
739E: 86 65       LDA   #$65
73A0: BD 5F 99    JSR   JMP_PRINT_STRING_LARGE_FONT  ;print CENTER THE LINE USING -FIRE- PRESS ADVANCE WHEN CENTERED
73A3: 86 04       LDA   #$04                 ; delay before calling function 
73A5: 8E 73 AB    LDX   #$73AB               ; address of function to call
73A8: 7E D0 66    JMP   $D066                ; JMP $D1E3 - allocate task

73AB: B6 C8 0C    LDA   rom_pia_dataa
73AE: 85 02       BITA  #$02                 ; has "ADVANCE" been pressed?
73B0: 26 F1       BNE   $73A3                ; yes, advance has been pressed, goto $73A3, wait until user takes finger off.

73B2: B6 C8 06    LDA   widget_pia_datab
73B5: 85 02       BITA  #$02                 ; FIRE RIGHT held?
73B7: 27 10       BEQ   $73C9                ; no, goto $73C9

; handle user pressing FIRE RIGHT to move message right
73B9: AE 4B       LDX   $000B,U              ; get address of horizontal position counter  
73BB: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1 ; read value of counter from X
73BE: AE 4B       LDX   $000B,U              ; get address of horizontal position counter again 
73C0: 4C          INCA                       ; increment horizontal position counter
73C1: 81 3A       CMPA  #$3A                 ; have we moved the text as far right as we are permitted to? 
73C3: 23 15       BLS   $73DA                ; no, goto $73DA 
73C5: 86 3A       LDA   #$3A                 ; otherwise, clamp counter to #$3A (58 decimal)
73C7: 20 11       BRA   $73DA                ; and write the updated counter value back to memory

; A = bits read from widget_pia_datab
73C9: 46          RORA                       ; move FIRE LEFT bit into carry  
73CA: 24 1D       BCC   $73E9                ; if FIRE LEFT is not held down, then goto $73E9

; handle user pressing FIRE LEFT to move message left
73CC: AE 4B       LDX   $000B,U              ; get address of horizontal position counter 
73CE: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1 ; read value of counter from X
73D1: AE 4B       LDX   $000B,U              ; get address of horizontal position counter again       
73D3: 4A          DECA                       ; decrement horizontal position counter
73D4: 81 13       CMPA  #$13                 ; have we moved the text as far left as we are permitted to?
73D6: 24 02       BCC   $73DA                ; no, goto $73DA
73D8: 86 13       LDA   #$13                 ; otherwise, clamp counter #$3A (58 decimal)

73DA: BD D0 AB    JSR   STA_NIB_X1           ; split A into 2 and write result to X (update horizontal counter)
73DD: A7 47       STA   $0007,U
73DF: 8D 2C       BSR   $740D                ; display message in new position
73E1: 86 10       LDA   #$10                 ; delay before calling function
73E3: 8E 73 B2    LDX   #$73B2               ; address of function to call
73E6: 7E D0 66    JMP   $D066                ; JMP $D1E3 - allocate task

73E9: B6 C8 0C    LDA   rom_pia_dataa
73EC: 85 02       BITA  #$02                 ; Has "ADVANCE" been pressed?  
73EE: 26 08       BNE   $73F8                ; yes, goto $73F8
73F0: 86 04       LDA   #$04                 ; delay before calling function
73F2: 8E 73 B2    LDX   #$73B2               ; address of function to call
73F5: 7E D0 66    JMP   $D066                ; JMP $D1E3 - allocate task

; Clear the screen and then create a task that waits for ADVANCE to be unpressed
73F8: BD D0 12    JSR   CLR_SCREEN1
73FB: 86 04       LDA   #$04                 ; delay before calling function
73FD: 8E 74 03    LDX   #$7403               ; address of function to call
7400: 7E D0 66    JMP   $D066                ; JMP $D1E3 - allocate task

7403: B6 C8 0C    LDA   rom_pia_dataa
7406: 85 02       BITA  #$02                 ; Has "ADVANCE" been pressed?
7408: 26 F1       BNE   $73FB                ; yes, goto $73FB
740A: 6E D8 0D    JMP   [$0D,U]              ; jump to return address saved @$7392

;
;
; Y = pointer to packed characters to draw (2 bytes per character, lower nibble of byte pair used to form a single byte)
; U = pointer to task
;    U+7 = pointer to screen address to begin printing characters 
;    U+9 = pointer to packed characters to print
;

740D: 0F D0       CLR   $D0                  ; clear "blit on odd pixel" flag
740F: AE 47       LDX   $0007,U              ; read X coordinate                
7411: 30 89 FF 00 LEAX  -$100,X              ; subtracts 1 from X component of screen address held in X register 
7415: 86 5A       LDA   #$5A                 ; width in bytes (= 180 pixels)
7417: C6 09       LDB   #$09                 ; height in pixels
7419: BD D0 1B    JSR   $D01B                ; JMP $DADF - clear rectangle to black
741C: AE 47       LDX   $0007,U              ; read X coordinate again
741E: 0F D0       CLR   $D0                  ; clear "blit on odd pixel" flag
7420: 10 AE 49    LDY   $0009,U              ; get address of characters to print
7423: C6 19       LDB   #$19                 ; maximum length of line is #$19 (25 decimal) characters
7425: 1E 12       EXG   X,Y                  ; X is now address of character to print, Y is screen address to print character at
7427: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1 ; unpack 2 bytes from X into A. A now is ordinal of character
742A: 1E 12       EXG   X,Y                  ; X is now screen address to print character at, Y is address of character to print
742C: BD 5F 93    JSR   $5F93                ; JMP $6023 - call BLIT_LARGE_CHARACTER
742F: 5A          DECB                       ; have we done all characters?
7430: 26 F3       BNE   $7425                ; no, goto $7425
7432: 39          RTS   
...
746A: BD D0 AB    JSR   STA_NIB_X1           ; split A into 2 and write result to X. Writes updated setting value to CMOS  
746D: 35 10       PULS  X
...
CALCULATE_GAME_SETTINGS_CHECKSUM:
...
753A: DE 15       LDU   $15                  ; U = address of Task
753C: ED 47       STD   $0007,U
753E: AF 49       STX   $0009,U
7540: 10 AF 4B    STY   $000B,U
7543: 35 06       PULS  A,B                  ; pull return address off stack
7545: ED 4D       STD   $000D,U              ; store return address in Task
...
754F: 8D 34       BSR   $7585                ; test FIRE UP bit
7551: 26 F4       BNE   $7547                ; if FIRE UP is pressed, goto $7547
7553: BD D0 54    JSR   $D054                ; JMP $D281 - reserve object metadata entry and call function
7556: 77 49       ; pointer to function
; Now X = pointer to an entry in the object metadata list
7558: EC 4D       LDD   $000D,U              ; get return address saved @$7545      
755A: ED 0D       STD   $000D,X              ; and store in object metadata
755C: EC 47       LDD   $0007,U              ; get task address saved @$753C            
755E: ED 07       STD   $0007,X              ; and store in object metadata
7560: EF 09       STU   $0009,X              ; store pointer to task in object metadata
7562: 9F D8       STX   $D8
7564: 86 99       LDA   #$99                 ; WHITE in current palette
7566: 97 CF       STA   $CF                  ; set text colour 
...
757A: BD 76 2A    JSR   $762A                ; clear rectangle to black
757D: BD 76 36    JSR   $7636                ; draw underline _ cursor
7580: 86 3A       LDA   #$3A
...
;
; Returns:
; Z flag == 0 if FIRE UP is not pressed
;

7585: B6 C8 04    LDA   widget_pia_dataa     ; read sticks
7588: 85 40       BITA  #$40                 ; test FIRE UP bit
758A: 39          RTS   

758B: B6 C8 04    LDA   widget_pia_dataa     ; read sticks
758E: 46          RORA                       ; put MOVE UP bit into carry
758F: 10 25 01 16 LBCS  $76A9                ; if MOVE UP is pressed, goto $76A9
7593: 46          RORA                       ; put MOVE DOWN bit into carry
7594: 10 25 00 F3 LBCS  $768B                ; if MOVE DOWN is pressed, goto $768B
7598: 8D EB       BSR   $7585                ; read FIRE UP status. 
759A: 27 D6       BEQ   $7572                ; FIRE UP is not pressed, goto $7572 
...
75F8: 8D 4B       BSR   $7645
75FA: 8D 2E       BSR   $762A                ; clear rectangle to black
75FC: 86 3A       LDA   #$3A
75FE: 8D 5D       BSR   $765D
7600: 9E DA       LDX   $DA
...
;
; Given a screen address stored at U+9, clear an 8 x 7 pixel rectangle to black.
;

CLEAR_8_BY_7_PIXEL_RECTANGLE_U:
762A: 34 06       PSHS  B,A
762C: AE 49       LDX   $0009,U             ; blitter destination
762E: CC 04 07    LDD   #$0407              ; width in bytes = 4 (8 pixels), height = 7
7631: BD D0 1B    JSR   $D01B               ; JMP $DADF - clear rectangle to black
7634: 35 86       PULS  A,B,PC ;(PUL? PC=RTS)

;
; Draw the underline _ cursor on the ATTRACT MODE MESSAGE screen 
;

DRAW_UNDERLINE_CURSOR:
7636: 86 99       LDA   #$99                 ; WHITE in current palette
7638: AE 49       LDX   $0009,U         
763A: A7 08       STA   $0008,X              ; write 2 pixels  
763C: A7 89 01 08 STA   $0108,X              ; write next 2 pixels on same row
7640: A7 89 02 08 STA   $0208,X              ; write next 2 pixels on same row
7644: 39          RTS   
...
766D: BD D0 AB    JSR   STA_NIB_X1           ; split A into 2 and write result to X
...
; wait a short while...
;
; Basically count down from #$2000 (8192 decimal) to 0 and then exit.
;

DELAY_FOR_WHILE:
7683: 8E 20 00    LDX   #$2000             ; Load X with 8192 decimal..
7686: 30 1F       LEAX  $-1,X              ; X--
7688: 26 FC       BNE   $7686                
768A: 39          RTS   

; Called when MOVE DOWN is pressed on the ATTRACT MODE MESSAGE screen
768B: 86 0A       LDA   #$0A
768D: 8D 7F       BSR   $770E
768F: 8D F2       BSR   $7683               ; wait a short while... 
7691: F6 C8 04    LDB   widget_pia_dataa    ; read joystick bits
7694: C5 02       BITB  #$02                ; test MOVE DOWN bit
7696: 10 27 FE FE LBEQ  $7598               ; if MOVE DOWN is not pressed, goto $7598
769A: 4A          DECA  
769B: 26 F2       BNE   $768F
...
; Called when MOVE UP is pressed on the ATTRACT MODE MESSAGE screen
76A9: 86 0A       LDA   #$0A
76AB: 8D 19       BSR   $76C6
76AD: 8D D4       BSR   $7683               ; wait a short while...
76AF: F6 C8 04    LDB   widget_pia_dataa    ; read joystick bits 
76B2: 56          RORB                      ; move bit 0 (MOVE UP bit flag) into carry  
76B3: 10 24 FE E1 LBCC  $7598               ; if MOVE UP is not pressed, goto $7598
76B7: 4A          DECA  
76B8: 26 F3       BNE   $76AD
...
7700: BD 76 2A    JSR   $762A               ; clear rectangle to black
...
7755: 86 FF       LDA   #$FF                ; delay before calling function
7757: 8E 77 5D    LDX   #$775D              ; address of function to call
775A: 7E D0 66    JMP   $D066               ; JMP $D1E3 - allocate task

775D: 86 82       LDA   #$82                ; delay before calling function 
775F: 8E 77 65    LDX   #$7765              ; address of function to call
...
D1B0: 85 04       BITA  #$04
D1B2: 26 03       BNE   $D1B7
D1B4: BD 5B 49    JSR   $5B49                ; JMP   $5E15
D1B7: 9E 33       LDX   $33
D1B9: 26 0C       BNE   $D1C7
D1BB: 9E 37       LDX   $37
D1BD: 27 17       BEQ   $D1D6
...
; Corrupt fields in the object state. 
DE3F: 34 04       PSHS  B
DE41: D6 85       LDB   $85                   
